...
public class ExhibitAPI {
  ...
  public Result update(Exhibit updatedExhibit) {
    ...
    try (EntityManager entityManager = emf.createEntityManager()) {
      ...
      // 如果属性值为 null, 保持原有值
      if (Objects.isNull(updatedExhibit.getTitle())) {
        updatedExhibit.setTitle(existingExhibit.getTitle());
      }
      if (Objects.isNull(updatedExhibit.getDescription())) {
        updatedExhibit.setDescription(existingExhibit.getDescription());
      }
      if (Objects.isNull(updatedExhibit.getImg())) {
        updatedExhibit.setImg(existingExhibit.getImg());
      }
      if (Objects.isNull(updatedExhibit.getNum())) {
        updatedExhibit.setNum(existingExhibit.getNum());
      }
      if (updatedExhibit.getCategoryId() == 0) {
        updatedExhibit.setCategoryId(existingExhibit.getCategoryId());
      }
